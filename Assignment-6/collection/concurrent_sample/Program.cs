...
namespace ConcurrentSample
{
    class Program
    {
        ...
        static void BlockingDemoSimple()
        {
            ...
            producer.Start(Tuple.Create<BlockingCollection<int>,
            ManualResetEventSlim>(sharedCollection, events[0]));
            ...
            consumer.Start(Tuple.Create<BlockingCollection<int>,
            ManualResetEventSlim>(sharedCollection, events[1]));
        ...
        static void BlockingDemo()
        {
            ...
            ManualResetEventSlim[] events = new
            ManualResetEventSlim[threadCount];
            ...
            for (int thread = 0; thread < threadCount >> 1; thread++)
            {
                ...
                {
                    var coll = ((Tuple<BlockingCollection<int>,
                    ManualResetEventSlim>)state).Item1;
                    var wait = ((Tuple<BlockingCollection<int>,
                    ManualResetEventSlim>)state).Item2;
                    var r = new Random();
                    for (int i = 0; i < 300; i++)
                    {
                ...
                producer.Start(Tuple.Create<BlockingCollection<int>,
                ManualResetEventSlim>(sharedCollection, events[thread]));
            }
            Thread.Sleep(500); // give the producers a headstart
            for (int thread = threadCount >> 1; thread < threadCount;
            thread++)
            {
